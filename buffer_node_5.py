...
class GetPolicy:
    ...
    # Define a function that adds exploration noise to the action
    ...
    # Define a function that does not add noise to the action
    def policy(self, state):
        ...
        # rospy.loginfo(type(sampled_actions))
        legal_action_vel = np.clip(
            sampled_actions[0], self.throttle_lower_bound, self.throttle_upper_bound)
        legal_action_str = np.clip(
            sampled_actions[1], self.steering_lower_bound, self.steering_upper_bound)
...
class Buffer:
    ...
    def save_buffer(self):
        ...
        buffer_list = [self.state_buffer,
                       self.action_buffer, self.next_state_buffer]
...
class Buffer_Node(object):
    ...
    def __init__(self):
        ...
        self.pose_sub_ = rospy.Subscriber(
            '/mocap/rigid_bodies/RigidBody_01/pose', PoseStamped, self.update_coords, queue_size=100)
        self.cmd_sub_ = rospy.Subscriber(
            '/qcar/user_command', Vector3Stamped, self.process_cmd, queue_size=10)
        self.cmd_pub_ = rospy.Publisher(
            '/critic_value', Vector3Stamped, queue_size=100)

        self.sampling_rate = rospy.Rate(5)  # Hz

        self.state_mean = [0.08157198, -0.01861664, -
                           0.02135922,  0.00041671, -0.02958437, 0.00024513]
        self.state_variance = [6.54200947e-02, 6.61160108e-01,
                               6.51017401e-03, 2.04472943e-02, 9.38314797e-01, 3.46092837e+01]
        ...
        self.buffer = Buffer(buffer_name, self.new_buffer,
                             self.records_per_run, self.num_states, self.num_actions)
        self.getPolicy = GetPolicy(
            network_name, self.num_states, self.num_actions)
...
#-------------------------------------------------------------------------------------------------
    ...
    def looping(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.speed = np.sqrt((self.x - self.x_prev) **
                                 2 + (self.z - self.z_prev)**2) * 5
            ...
            if(self.buffer.buffer_counter < self.records_per_run):
                ...
                norm_state = (self.state - self.state_mean) / \
                    np.sqrt(self.state_variance)

                tf_prev_state = tf.expand_dims(
                    tf.convert_to_tensor(norm_state), 0)
                ...
                critic_value = self.getPolicy.critic_model(
                    [tf_prev_state, tf_action])
                rospy.loginfo(
                    "Critic Value at current state = " + str(critic_value))
    ...
    # Calculate vehicle states from GPS
    def update_coords(self, pose_msg):
        ...
        self.yaw = math.atan2(2 * x1 * z1 - 2 * w1 *
                              y1, -1 * (1 - 2 * x1**2 - 2 * y1**2))
