...
class Learner:
    def __init__(self, buffer_name, network_name, new_network, buffer_capacity=100000, batch_size=64, num_states=2, num_actions=2):
            ...
            try:
                ...
                for i in range(len(buffer_list)):
                    ...
                    if(i % 4 == 0):
                        ...
                        self.state_buffer[self.buffer_counter:self.buffer_counter+list_length,:] = buffer_list[i]
                    elif(i % 4 == 1):
                        self.action_buffer[self.buffer_counter:self.buffer_counter+list_length] = buffer_list[i]
                    elif(i % 4 == 2):
                        self.reward_buffer[self.buffer_counter:self.buffer_counter+list_length,:] = buffer_list[i]
                    else:
                        self.next_state_buffer[self.buffer_counter:self.buffer_counter+list_length,:] = buffer_list[i]

            
                    print("i is " + str(i) + " shape of buffer_list[i] is " + str(np.shape(buffer_list[i])))
        ...
        if(new_network == False):
            ...
            self.target_critic.load_weights(self.network_name + str("/target_critic.h5"))
            self.target_actor.load_weights(self.network_name + str("/target_actor.h5"))
    ...
    def update(self,state_batch,action_batch,reward_batch,next_state_batch):
        ...
        with tf.GradientTape() as tape:
            ...
            #converted_critic_loss = critic_loss.eval(session=tf.Session())
            #output moving average critic loss

            #self.critic_loss_buffer = np.roll(self.critic_loss_buffer,1)
            #self.critic_loss_buffer[0] = converted_critic_loss
            #print("Critic Loss = " + str(np.mean(self.critic_loss_buffer, axis=0)))
    ...
    def learn(self):
        ...
        crit_loss, act_loss = self.update(state_batch, action_batch, reward_batch, next_state_batch)
    ...
    def save_models(self):
        self.actor_model.save_weights(self.network_name + str("/actor_model.h5"))
        self.critic_model.save_weights(self.network_name + str("/critic_model.h5"))
        self.target_critic.save_weights(self.network_name + str("/target_critic.h5"))
        self.target_actor.save_weights(self.network_name + str("/target_actor.h5"))
...
def main():
    ...
    learner = Learner(buffer_name, network_name, new_network, 20000, 256, num_states, num_actions)
    noise_object = OUActionNoise(mean=np.zeros(1), std_deviation=float(noise_std) * np.ones(1)) 
    ...
    while(True):
        ...
        if(learning_index < learning_iterations):
            if(learning_index % decay_steps ==  5000):
                ...
                learner.actor_optimizer = tf.keras.optimizers.Adam(learner.actor_lr)
                learner.critic_optimizer = tf.keras.optimizers.Adam(learner.critic_lr)
